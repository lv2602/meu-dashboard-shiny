...
suppressPackageStartupMessages({
  ...
  library(sf); library(dplyr); library(rlang); library(tidyr); library(magrittr)
...
normalize_name <- function(x) x |> as.character() |> trimws() |> toupper() |> stringi::stri_trans_general("Latin-ASCII")
canon <- function(v) stringi::stri_trans_general(toupper(trimws(v)), "Latin-ASCII")
as_num <- function(x) {
  if (is.numeric(x)) return(x);
  x <- as.character(x); x <- gsub(",", ".", x); suppressWarnings(as.numeric(x))
}
rename_by_alias <- function(df, aliases) {
  ...
  for (target in names(aliases)) {
    ...
    if (length(hit)) {
      names(df)[hit[1]] <- target; c_map[hit[1]] <- canon(target)
    }
  }
  df
}
...
make_pct_axis <- function(x) {
  ...
  span <- hi - lo
  if (span < 30) {
    mid <- (lo + hi) / 2; lo <- max(0, floor(mid - 20)); hi <- min(100, ceiling(mid + 20))
  }
...
load_bairros_shapefile <- function() {
  shp <- getOption("edu_shp_path", "Limite_de_Bairros.shp")
  ...
  else if (sf::st_crs(sfobj)$epsg != 4326) sfobj <- sf::st_transform(sfobj, 4326)
...
# ---- Dados Excel ----------------------------------------------------------
DATA_XLSX <- getOption("edu_data_path", "indicadores_rj_certo.xlsx")
...
# Séries Históricas
s_hist <- pick("SERIES HISTORICAS","SERIES_HISTORICAS","HISTORICAS","SERIES")
d <- list(
...
read_factor_onesheet <- function(sheet, prefer_names, out_col) {
  ...
  df <- tibble(key = raw$key, v = val) |> group_by(key) |> summarise(v = if (all(is.na(v))) NA_real_ else mean(v, na.rm = TRUE), .groups = "drop")
...
# Séries Históricas do Excel
hist_df <- NULL
if (!is.na(s_hist)) {
  hist_raw <- suppressWarnings(readxl::read_excel(DATA_XLSX, sheet = s_hist, col_names = FALSE)) |> ensure_tibble()
  if (nrow(hist_raw) >= 8) {
    # Extrair anos (assumindo estrutura fixa)
    extract_year <- function(s) sub(".*(\\d{4})$", "\\1", trimws(as.character(s)))
    ideb_years <- sapply(hist_raw[1, 2:5], extract_year)
    ideb_vals <- as_num(hist_raw[2, 2:5])
    port_years <- sapply(hist_raw[4, 2:5], extract_year)
    port_vals <- as_num(hist_raw[5, 2:5])
    mat_years <- sapply(hist_raw[7, 2:5], extract_year)
    mat_vals <- as_num(hist_raw[8, 2:5])
    # Assumir anos iguais
    years <- as.integer(ideb_years)
    hist_df <- tibble(
      year = years,
      IDEB = ideb_vals,
      `SAEB Português` = port_vals,
      `SAEB Matemática` = mat_vals
    ) %>% pivot_longer(cols = -year, names_to = "indicador", values_to = "media") %>% filter(is.finite(media))
  }
}
if (is.null(hist_df) || !nrow(hist_df)) {
  # Fallback para hardcoded se não carregou
  hist_df <- tibble(
    year = c(2017, 2019, 2021, 2023),
    IDEB = c(3.20, 3.62, 3.78, 3.22),
    `SAEB Português` = c(259.12, 274.73, 259.33, 257.71),
    `SAEB Matemática` = c(257.87, 270.59, 252.03, 253.19)
  ) %>% pivot_longer(cols = -year, names_to = "indicador", values_to = "media")
}
# Labels
label_map <- c(
...
make_school_factor_df <- function(factor_var) {
  ...
  normalize_pct <- function(v) {
    if (length(v) && any(is.finite(v))) {
      ...
      if (is.finite(mx) && mx <= 1.2) return(v * 100)
    }
    v
  }
        ...
        fluidRow(
          ...
          column(4,
                 div(class="card border-info mb-3",
                     ...
                     downloadButton("download_mini_scatter", "Baixar PNG", class = "btn-sm"))
          ),
          column(4,
                 div(class="card border-warning mb-3",
                     ...
                     downloadButton("download_rank_table", "Baixar CSV", class = "btn-sm")))
        )
      )
    )
  ),
      ...
      mainPanel(
        ...
        div(class="card",
            div(class="card-header",
                h4("Mapa Educacional - Rio de Janeiro", style="margin:0;"),
                textOutput("map_status")),
            div(class="card-body", style="padding:0;",
                plotlyOutput("map_plotly", height = 820)),
            downloadButton("download_map_plotly", "Baixar PNG", class = "btn-sm")
        )
      )
    )
  ),
    ...
    sidebarLayout(
      ...
      mainPanel(
        ...
        fluidRow(
          column(6,
                 div(class="card",
                     div(class="card-header", h4("Dispersão (escolas)", style="margin:0;")),
                     div(class="card-body", plotOutput("scatter", height=450)),
                     downloadButton("download_scatter", "Baixar PNG", class = "btn-sm"))),
          column(6,
                 div(class="card",
                     div(class="card-header", h4("Matriz de Correlação (Spearman) — escolas", style="margin:0;")),
                     div(class="card-body", plotOutput("corrmat", height=450)),
                     downloadButton("download_corrmat", "Baixar PNG", class = "btn-sm")))
        )
      )
    )
  ),
  tabPanel(
    ...
    sidebarLayout(
      sidebarPanel(
        width = 3,
        h5("Configurações"),
        selectInput("time_set","Conjunto de Indicadores:",
                    choices = c("Todos"="all", "Apenas IDEB"="ideb", "SAEB Mat e Port"="saeb"),
                    selected = "all"),
        selectInput("time_indicators","Indicadores:",
                    choices = unique(hist_df$indicador),
                    selected = unique(hist_df$indicador), multiple = TRUE)
      ),
      mainPanel(
        width = 9,
        div(class="card",
            div(class="card-header", h4("Evolução dos Indicadores Educacionais no Rio de Janeiro (médias por ano)", style="margin:0;")),
            div(class="card-body", plotOutput("time_series_plot", height = 500)),
...
server <- function(input, output, session) {
  ...
  compute_bairro_means <- function(var) {
    ...
    pts_sf <- sf::st_as_sf(pts, coords = c("lon","lat"), crs = 4326, remove = FALSE)
    pts_in <- suppressWarnings(sf::st_join(pts_sf, polys[, c("nome_bairro")], join = sf::st_within, left = FALSE))
    if (!nrow(pts_in)) return(NULL)
    agg <- pts_in %>% sf::st_drop_geometry() %>% group_by(nome_bairro) %>% summarise(map_val = mean(valor, na.rm = TRUE), n_escolas = dplyr::n(), .groups = "drop")
    df <- polys %>% dplyr::select(nome_bairro) %>% sf::st_drop_geometry() %>% left_join(agg, by = "nome_bairro")
    if (any(is.na(df$map_val))) {
      polys_utm <- sf::st_transform(polys, 31983); cent <- sf::st_centroid(sf::st_geometry(polys_utm))
      have <- which(!is.na(df$map_val)); miss <- which(is.na(df$map_val))
      if (length(have) > 0 && length(miss) > 0) {
        D <- as.matrix(sf::st_distance(cent[miss], cent[have])); k <- pmin(3, length(have))
  ...
  mini_scatter_plot <- reactive({
    df <- make_school_corr_df2(input$educ_var, input$factor_var); if (nrow(df) < 2) return()
    p <- ggplot(df, aes(fator, indicador)) + geom_point(alpha=0.75, size=2) + geom_smooth(method="lm", se=FALSE, linewidth=0.9) +
    ...
    if (input$factor_var %in% c("pctsalasar","pctturmastempointegral","aprovacao3serie")) {
      ...
      p <- p + scale_x_continuous(limits = ax$lims, breaks = ax$breaks, labels = scales::number_format(suffix = "%", accuracy = 1))
    }
    p
  })
  ...
  rank_data <- reactive({
    switch(input$educ_var,
           enem_media = ensure_tibble(enem_df) %>% mutate(enem_media = as_num(enem_media)) %>% filter(is.finite(enem_media)) %>% arrange(desc(enem_media)) %>% slice_head(n = 10) %>% mutate(ranking = row_number()) %>% transmute(`#`=ranking, Escola=as.character(nome_escola), Valor=round(enem_media)),
           ideb_anos_finais = ensure_tibble(ideb_df) %>% mutate(ideb_anos_finais = as_num(ideb_anos_finais)) %>% filter(is.finite(ideb_anos_finais)) %>% arrange(desc(ideb_anos_finais)) %>% slice_head(n = 10) %>% mutate(ranking = row_number()) %>% transmute(`#`=ranking, Escola=as.character(nome_escola), Valor=ideb_anos_finais),
           saeb_portugues = ensure_tibble(saeb_port_df) %>% mutate(saeb_portugues = as_num(saeb_portugues)) %>% filter(is.finite(saeb_portugues)) %>% arrange(desc(saeb_portugues)) %>% slice_head(n = 10) %>% mutate(ranking = row_number()) %>% transmute(`#`=ranking, Escola=as.character(nome_escola), Valor=saeb_portugues),
           saeb_matematica = ensure_tibble(saeb_mat_df) %>% mutate(saeb_matematica = as_num(saeb_matematica)) %>% filter(is.finite(saeb_matematica)) %>% arrange(desc(saeb_matematica)) %>% slice_head(n = 10) %>% mutate(ranking = row_number()) %>% transmute(`#`=ranking, Escola=as.character(nome_escola), Valor=saeb_matematica)
    )
  })
  output$rank_table <- renderReactable({
    ...
    reactable(
      ...
      columns = list(
        `#` = colDef(name="#", width=44, align="center", cell=function(v) if (v %in% 1:3) paste0(v, "\u00BA") else as.character(v), style=function(v) if (v %in% 1:3) list(fontWeight="700", color="#d4af37") else NULL),
        Escola = colDef(name="Escola", minWidth=200, align="left", style=function(val, i) if (i %in% which(dados$`#` %in% 1:3)) list(fontWeight="600") else NULL),
  ...
  output$rank_legend <- renderUI({
    ...
    tags$div(style="margin-top:6px; font-size:12px; color:#555;",
  ...
  map_plot <- reactive({
    ...
    if (!is.null(bairro_means)) {
      ...
      df$.val_map <- NA_real_; df$.n_escolas = 0L
    }
    fmt_poly <- function(x) ifelse(is.na(x), "s/ dado", if (var=="ideb_anos_finais") sprintf("%.1f", x) else sprintf("%.0f", x))
    df$.hover <- sprintf("<b>Bairro:</b> %s<br>%s (média): <b>%s</b><br>Escolas usadas: %s", df$nome_bairro, educ_label, fmt_poly(df$.val_map), ifelse(is.na(df$.n_escolas), 0, df$.n_escolas))
    p <- suppressWarnings(
      ggplot(df) + geom_sf(aes(fill = .val_map, text = .hover), color="grey30", linewidth=0.25, alpha=0.85) +
        scale_fill_gradient(low = "#d7191c", high = "#1a9641", na.value="grey85", name=paste0(educ_label, " (bairros)")) +
        coord_sf(expand = FALSE) + theme_void(base_size=12) + ggtitle(paste("Mapa Educacional RJ –", educ_label)) +
        theme(plot.title=element_text(face="bold", hjust=0.5, margin=margin(b=8)), legend.position="right")
    )
    if (isTRUE(input$show_pins) && !is.null(schools_df) && nrow(schools_df) > 0) {
      ...
      pts$hover_pt <- if (!all(is.na(pts$cep_fmt))) {
        sprintf("<b>%s</b><br>%s: <b>%s</b><br>CEP: %s<br>Lat/Lon: %.6f, %.6f", pts$nome_escola, educ_label, fmt_school(pts$valor), pts$cep_fmt, pts$lat, pts$lon)
      } else {
        sprintf("<b>%s</b><br>%s: <b>%s</b><br>Lat/Lon: %.6f, %.6f", pts$nome_escola, educ_label, fmt_school(pts$valor), pts$lat, pts$lon)
      }
      p <- p + geom_point(data = pts, mapping = aes(x = lon, y = lat, fill = valor, text = hover_pt), shape = 24, size = 3.2, stroke = 0.6, color = "black", alpha = 0.95, show.legend = TRUE) +
  ...
  output$map_plotly <- renderPlotly({
    p <- map_plot()
    plotly::ggplotly(p, tooltip = "text") |> plotly::config(displayModeBar = FALSE, scrollZoom = TRUE) |>
      plotly::layout(margin = list(l=10, r=10, t=40, b=10), hoverlabel = list(bgcolor = "rgba(255,255,255,0.95)", bordercolor = "rgba(0,0,0,0.1)", font = list(size = 12)), hovermode = "closest")
  })
  # Dispersão
  scatter_plot <- reactive({
    ...
    p <- ggplot(df, aes(fator, indicador)) + geom_point(alpha=0.75, size=2.2) +
      labs(x=label_factor[[input$corr_factor_var]], y=label_map[[input$corr_educ_var]],
           title=paste("Correlação (escolas) — Pearson:", sprintf("%.3f", r_pear), " | Spearman:", sprintf("%.3f", r_spear))) +
      theme_minimal(12) + theme(plot.title=element_text(face="bold", size=13))
    if (isTRUE(input$show_regression)) p <- p + geom_smooth(method="lm", se=isTRUE(input$show_confidence), linewidth=1.2)
  ...
  # Matriz Spearman
  corrmat_plot <- reactive({
    pick_factor <- function(keyname) {
      df <- make_school_factor_df(keyname); if (!nrow(df)) return(NULL)
      pretty <- switch(keyname, "aprovacao3serie"="Aprovação 3ª (%)","alunoperturma"="Aluno/turma","alunoporprofessor"="Aluno/prof.",
    ...
    if (!length(lst)) return(ggplot() + theme_void() + annotate("text", 0, 0, label = "Sem dados de fatores suficientes para a matriz.", size = 5, fontface = "bold"))
    ...
    numdf <- numdf[, keep, drop = FALSE]
    if (ncol(numdf) < 2) {
      msg <- if (ncol(numdf) == 1) paste0("Apenas 1 fator com dados suficientes: ", colnames(numdf)[1]) else "Sem fatores com dados suficientes para calcular correlação."
    ...
    if (!is.matrix(M) || all(!is.finite(M))) return(ggplot() + theme_void() + annotate("text", 0, 0, label = "Não foi possível calcular a correlação (muitos NAs).", size = 5, fontface = "bold"))
    dfm <- as.data.frame(as.table(M)); names(dfm) <- c("VarY","VarX","r")
    ggplot(dfm, aes(VarX, VarY, fill = r)) + geom_tile(color="white", size=0.5) +
      geom_text(aes(label = ifelse(is.finite(r), sprintf("%.2f", r), "NA")), size = 3) +
      scale_fill_gradient2(limits = c(-1,1), low = "#d73027", mid = "#ffffbf", high = "#1a9850", midpoint = 0, name = "r") +
      coord_equal() + theme_minimal(12) +
      theme(axis.text.x = element_text(angle = 45, hjust = 1), legend.position = "right", legend.title = element_text(face = "bold"), panel.grid = element_blank()) +
      labs(x = NULL, y = NULL)
  })
  ...
  # Séries Temporais
  observe({
    ch <- unique(hist_df$indicador)
    sel <- switch(input$time_set, "all" = ch, "ideb" = "IDEB", "saeb" = ch[grepl("SAEB", ch)])
    updateSelectInput(session, "time_indicators", choices = ch, selected = sel)
  })
  filtered_hist <- reactive({
    if (length(input$time_indicators) == 0) return(hist_df)
    hist_df %>% filter(indicador %in% input$time_indicators)
  })
  time_series_plot <- reactive({
    df <- filtered_hist()
    if (nrow(df) == 0) return(ggplot() + theme_void() + annotate("text", 0, 0, label = "Selecione pelo menos um indicador.", size = 5, fontface = "bold"))
    ggplot(df, aes(x = year, y = media, color = indicador)) +
      geom_line(linewidth = 1.2) +
      geom_point(size = 3) +
      geom_text(aes(label = sprintf("%.1f", media)), colour = "black", vjust = -1, size = 4, fontface = "bold") +
      labs(title = "Evolução dos Indicadores Educacionais no Rio de Janeiro (médias por ano)",
           x = "Ano", y = "Média", color = "Indicador") +
      scale_color_manual(values = c("IDEB" = "#1a9641", "SAEB Português" = "#a6d96a", "SAEB Matemática" = "#d7191c")) +
      theme_minimal(14) + theme(legend.position = "bottom", plot.title = element_text(face = "bold", hjust = 0.5))
  })
  output$time_series_plot <- renderPlot({
    time_series_plot()
  })
  # Downloads
  output$download_mini_scatter <- downloadHandler(
    filename = function() { "mini_scatter.png" },
    content = function(file) { p <- mini_scatter_plot(); ggsave(file, plot = p, device = "png", width = 6, height = 4, bg = "white") }
  )
  output$download_rank_table <- downloadHandler(
    filename = function() { "top10_escolas.csv" },
    content = function(file) { write.csv(rank_data(), file, row.names = FALSE) }
  )
  output$download_map_plotly <- downloadHandler(
    filename = function() { "mapa_educacional.png" },
    content = function(file) { p <- map_plot(); ggsave(file, plot = p, device = "png", width = 10, height = 8, bg = "white") }
  )
  output$download_scatter <- downloadHandler(
    filename = function() { "dispersao_escolas.png" },
    content = function(file) { p <- scatter_plot(); ggsave(file, plot = p, device = "png", width = 8, height = 6, bg = "white") }
  )
  output$download_corrmat <- downloadHandler(
    filename = function() { "matriz_correlacao.png" },
    content = function(file) { p <- corrmat_plot(); ggsave(file, plot = p, device = "png", width = 8, height = 6, bg = "white") }
  )
  output$download_time_series <- downloadHandler(
    filename = function() { "evolucao_indicadores.png" },
    content = function(file) { p <- time_series_plot(); ggsave(file, plot = p, device = "png", width = 10, height = 6, bg = "white") }
  )
}
