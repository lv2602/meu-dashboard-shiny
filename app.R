...
# Caminhos de dados (simplificado para servidor)
choose_base <- function() {
  if (dir.exists("data")) return("data")
  stop("Não encontrei a pasta 'data' ao lado do app.", call. = FALSE)
}
BASE <- choose_base()

options(
  edu_data_path = file.path(BASE, "indicadores_rj_certo.xlsx"),
  edu_shp_path  = file.path(BASE, "Limite_de_Bairros.shp")
)
...
load_bairros_shapefile <- function() {
  rds <- file.path("data", "bairros_simplificado.rds")
  if (file.exists(rds)) {
    sfobj <- readRDS(rds)
  ...
  shp <- getOption("edu_shp_path", file.path("data","Limite_de_Bairros.shp"))
  if (!file.exists(shp)) {
    stop("Shapefile (.shp) não encontrado em: ", shp,
         "\nColoque os arquivos na pasta 'data' do projeto.", call. = FALSE)
  }
...
# Excel
DATA_XLSX <- getOption("edu_data_path", file.path("data", "indicadores_rj_certo.xlsx"))
if (!file.exists(DATA_XLSX)) stop("Excel não encontrado em: ", DATA_XLSX, "\nColoque o arquivo na pasta 'data'.", call. = FALSE)
  ...
  output$map_plotly <- renderPlotly({
    ...
    for (i in seq_along(g$x$data)) {
      ...
      if (!is.null(tr$fill) && tr$fill %in% c("toself","tonextx","tonexty")) {
        g$x$data[[i]]$hoveron       <- "fills+points"
        g$x$data[[i]]$hoverinfo     <- "text"
        g$x$data[[i]]$hovertemplate <- "%{text}<extra></extra>"
      }
      if (!is.null(tr$mode) && grepl("lines", tr$mode)) {
    ...
    htmlwidgets::onRender(g, "
      function(el, x) {
        var gd = el; var factor = 0.5;
        gd.on('plotly_doubleclick', function() {
          var L = gd._fullLayout; if(!L || !L.xaxis || !L.yaxis) return;
          var xr = L.xaxis.range.slice(), yr = L.yaxis.range.slice();
          var xm = (xr[0] + xr[1]) / 2, ym = (yr[0] + yr[1]) / 2;
          var xspan = (xr[1] - xr[0]) - (xr[1] - xr[0]) * (1 - factor);
          var yspan = (yr[1] - yr[0]) - (yr[1] - yr[0]) * (1 - factor);
          Plotly.relayout(gd, {'xaxis.range':[xm - xspan/2, xm + xspan/2],
                               'yaxis.range':[ym - yspan/2, ym + yspan/2]});
          return false;
        });
      }")
  })
